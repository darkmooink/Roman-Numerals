import { decimalToRoman, romanToDecimal } from "./Roman-Numerals";
// I = 1
// V = 5
// X = 10
// L = 50
  ...
  const dataInput = 0, dataromanput = 1
    testData.forEach((data=>{
      it(`The number ${data.decimal} should convert into ${data.roman}`, () => {
        expect(decimalToRoman(data.decimal)).toBe(data.roman);
      });
    }))
  });
describe("Convert Roman Numerals to decimal numbers", () => {
  const dataInput = 0, dataromanput = 1
    testData.forEach((data=>{
      it(`The number ${data.roman} should convert into ${data.decimal}`, () => {
        expect(romanToDecimal(data.roman)).toBe(data.decimal);
      });
    }))
  });
